...
export class ListToDoPage extends BasePage {
    ...
    async clickAddNewToDoButton(): Promise<void> {
        await this.page
            .locator(ListToDoPageLocators.addNewToDoButtonXpath)
            .nth(0)
            .click();
    }
    ...
    async markLikeDoneToDo(number: number): Promise<void> {
        await this.page
            .locator(ListToDoPageLocators.unmarkedLikeDoneToDoCheckBoxXpath)
            .nth(Number(number))
            .click();
    }
    ...
    async unmarkLikeDoneToDoButton(number: number): Promise<void> {
        await this.page
            .locator(ListToDoPageLocators.markedLikeDoneToDoCheckBoxXpath)
            .nth(Number(number))
            .click();
    }
    ...
    async clickDeleteToDoButtonByIndex(number: number): Promise<void> {
        await this.page
            .locator(ListToDoPageLocators.deleteToDoButtonXpath)
            .nth(Number(number))
            .click();
    }
    ...
    async typeNewToDo(newToDo: string): Promise<void> {
        await this.page
            .locator(ListToDoPageLocators.createNewToDoFieldXpath)
            .fill(newToDo);
    }

    async clickCreateToDoButton(): Promise<void> {
        await this.page
            .locator(ListToDoPageLocators.createNewToDoButtonXpath)
            .click();
    }

    async goBackToListOfToDo(): Promise<void> {
        await this.page
            .locator(ListToDoPageLocators.goBackToToDoListButtonXpath)
            .click();
    }

    async deleteAllToDo(): Promise<void> {
        const deleteButtons = this.page.locator(
            ListToDoPageLocators.deleteToDoButtonXpath,
        );
    ...
    async markLikeDoneAllToDo(): Promise<void> {
        const LocatorsMarkToDoCheckBoxes = this.page.locator(
            ListToDoPageLocators.unmarkedLikeDoneToDoCheckBoxXpath,
        );
        const markToDoCheckBoxes =
            await LocatorsMarkToDoCheckBoxes.elementHandles();
    ...
    async unmarkLikeDoneAllToDo(): Promise<void> {
        const LocatorsMarkToDoCheckBoxes = this.page.locator(
            ListToDoPageLocators.markedLikeDoneToDoCheckBoxXpath,
        );
        const markToDoCheckBoxes =
            await LocatorsMarkToDoCheckBoxes.elementHandles();
        for (const markToDoCheckBox of markToDoCheckBoxes) {
